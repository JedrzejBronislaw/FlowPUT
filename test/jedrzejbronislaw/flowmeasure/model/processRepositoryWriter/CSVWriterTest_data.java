...
public class CSVWriterTest_data {
	...
	public void firstLineHeader() {
		...
		assertEquals(6, firstLine.length);
		...
		assertEquals("", firstLine[4]);
		assertTrue(firstLine[5].startsWith(ProcessRepositoryCSVWriter.FLOW_COLUMNNAME));
	}
	...
	public void secondLineHeader() {
		...
		assertEquals(7, secondLine.length);
	...
	public void firstLineData() {
		...
		assertEquals(ProcessRepositoryCSVWriter.processTime(startTime.plusSeconds(0), startTime), line[2]);
	...
	public void secondLineData() {
		...
		assertEquals(ProcessRepositoryCSVWriter.processTime(startTime.plusSeconds(1), startTime), line[2]);
		
		assertEquals("1", line[3]);
		assertEquals("2", line[4]);
		assertEquals(1/pulsePerLitre[0], Float.parseFloat(line[5]), deltaFlow);
		assertEquals(2/pulsePerLitre[1], Float.parseFloat(line[6]), deltaFlow);
	}
	...
	public void thirdLineData() {
		...
		assertEquals(ProcessRepositoryCSVWriter.processTime(startTime.plusSeconds(2), startTime), line[2]);
		
		assertEquals("2", line[3]);
		assertEquals("4", line[4]);
		assertEquals(2/pulsePerLitre[0], Float.parseFloat(line[5]), deltaFlow);
		assertEquals(4/pulsePerLitre[1], Float.parseFloat(line[6]), deltaFlow);
	}
